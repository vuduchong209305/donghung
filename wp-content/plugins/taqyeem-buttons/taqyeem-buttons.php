...
function tie_taqyeem_save_review_button_fields( $fields ){
	$custom_meta_fields = array(
		'taq_review_button_enable' => array(
...
function taq_buttons_admin_register() {
	...
	if( true === $load_fontawesome ){
		wp_enqueue_style ( 'taqyeem-admin-fontawesome' );
	}
}
	...
	<div id="taq-button-tab" class="tab_content taq-tabs-wrap">
		...
		<div class="taqyeem-item">
			...
			<?php
				taqyeem_option(
					array(
						'name' => __( "Don't include the FontAwesome icon set files" ,"taq" ),
...
function tie_taqyeem_button_output( $output ){
	...
	if( ! empty( $get_meta['taq_review_button_enable'][0] ) ){
		...
		$button_text     = ! empty( $get_meta['taq_review_button_text'][0] )     ? $get_meta['taq_review_button_text'][0]  : '';
		$button_icon     = ! empty( $get_meta['taq_button_icon'][0] )            ? $get_meta['taq_button_icon'][0]         : '';
		$button_color    = ! empty( $get_meta['taq_review_button_color'][0] )    ? $get_meta['taq_review_button_color'][0] : '';
		$button_size     = ! empty( $get_meta['taq_review_button_size'][0] )     ? $get_meta['taq_review_button_size'][0]  : '';
		$button_shape    = ! empty( $get_meta['taq_review_button_shape'][0] )    ? $get_meta['taq_review_button_shape'][0] : '';
		$button_type     = ! empty( $get_meta['taq_review_button_type'][0] )     ? $get_meta['taq_review_button_type'][0]  : '';
		$button_url      = ! empty( $get_meta['taq_review_button_url'][0] )      ? $get_meta['taq_review_button_url'][0]   : '';
		$button_target   = ! empty( $get_meta['taq_review_button_target'][0] )   ? ' target="_blank"' : '';
		$button_nofollow = ! empty( $get_meta['taq_review_button_nofollow'][0] ) ? ' rel="nofollow"'  : '';

		$button_class = ' taq-'.$button_size.' taq-'.$button_shape.' taq-'.$button_type;

		if( $button_icon == 'fa none'){
			$button_class .= ' without-icon';
		}
...
function taqyeem_buttons_addon_options(){
	...
	<div id="taq-button-wrapper">
		...
		taqyeem_options_items(
			array(
				'name' => __( 'Open links in a new window ?', 'taq' ),
		...
		taqyeem_options_items(
			array(
				'name'    => __( 'Button Size', 'taq' ),
				'id'      => 'taq_review_button_size',
				'type'    => 'radio',
				'options' => array(
					'medium' => __( 'Medium', 'taq' ),
					'small'  => __( 'Small',  'taq' ),
					'large'  => __( 'Large',  'taq' ),
				)));
		...
		taqyeem_options_items(
			array(
				'name'    => __( 'Button Shape', 'taq' ),
				'id'      => 'taq_review_button_shape',
				'type'    => 'radio',
				'options' => array(
					'square' => __( 'Square', 'taq' ),
					'round'  => __( 'Round',  'taq' ),
					'pill'   => __( 'Pill',   'taq' ),
				)));
		...
		taqyeem_options_items(
			array(
				'name'    => __( 'Button Type', 'taq' ),
				'id'      => 'taq_review_button_type',
				'type'    => 'radio',
				'options' => array(
					'flat'     => __( 'Flat',     'taq' ),
					'gradient' => __( 'Gradient', 'taq' ),
				)));
		?>
